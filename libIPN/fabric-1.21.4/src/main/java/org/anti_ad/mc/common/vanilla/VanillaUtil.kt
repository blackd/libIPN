...
import org.anti_ad.mc.alias.util.Util
...
object VanillaUtil {
    ...
    //fun lastFrameDuration(): Float = Vanilla.mc().renderTickCounter.lastFrameDuration
...
//  }
    ...
    fun getResourceAsString(identifier: String): String? = tryCatch {
        Vanilla.resourceManager().getResource(Identifier.tryParse(identifier)).get().inputStream?.reader()?.readText()
    }
