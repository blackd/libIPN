...
// this was here because the alias system can't handle
// direct usage of HoverEventAction.SHOW_TEXT in
// .withHoverEvent(HoverEvent(HoverEventAction.SHOW_TEXT

// Since 1.21.5 it's here because if the alias system inability to
// handle internal types like HoverEvent.ShowText

fun createHoverEventText(text: String): HoverEvent {
    return HoverEvent.ShowText(Text.literal(text))
}
...
fun fromSerializedJson(json: String): MutableText? {
    ...
    val text =  TextCodecs.CODEC.parse(DynamicRegistryManager.EMPTY.getOps(JsonOps.INSTANCE), jsonElement)
    /*return TextSerializer.fromJson(json, DynamicRegistryManager.EMPTY)*/
    return text.partialOrThrow as MutableText?
}
